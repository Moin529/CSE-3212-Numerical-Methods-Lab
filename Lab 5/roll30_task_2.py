...
OUTPUT_DIR = Path(__file__).resolve().parent / "output2"
...
def write_table(method, h, t_values, y_num):
    ...
    csv_path = OUTPUT_DIR / f"task2_{method.lower()}_h{h:.4f}.csv"
...
def main():
    ...
    y0 = 1.0
    t_end = 5.0
    step_sizes = [0.1, 0.005, 0.001, 0.0005, 0.0001]
    ...
    t_exact = np.linspace(t0, t_end, 1200)
    ...
    ax.set_title("Numerical solutions for dy/dt = y sin^3(t)")
    ax.set_xlabel("t")
    ax.set_ylabel("y(t)")
    ax.grid(True, alpha=0.35)
    ax.legend(loc="best", fontsize=8)
    plt.tight_layout()
    plot_path = OUTPUT_DIR / "task2_combined_plot.png"
